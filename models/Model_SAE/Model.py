...
class SAEModel(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        ...
        # elif self.is_training_mode and not ask_override and self.pretrain:
            # always ask if pretrain only if normal training not start
           # self.options['pretrain'] = io.input_bool ("\nStill Pretrain the model? (y/n, ?:help skip:n) : ", False, help_message="Pretrain the model with large amount of various faces. This technique may help to train the fake with overly different face shapes and light conditions of src/dst data. Face will be look more like a morphed. To reduce the morph effect, some model files will be initialized but not be updated after pretrain: LIAE: inter_AB.h5 DF: encoder.h5. The longer you pretrain the model the more morphed face will look. After that, save and run the training again.")
